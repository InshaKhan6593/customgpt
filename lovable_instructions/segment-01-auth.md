...
**Context:** WebletGPT is a Next.js 15 SaaS platform where developers build and monetize AI agents (Weblets) and users chat with them. We are building the authentication flow using Auth.js (NextAuth v5) with Magic Links (Passwordless Email OTP).
...
**Purpose:** The main entry point for users to request a Magic Link.
...
**Logic / State:** 
*   When the email form is submitted, the UI should swap to a "Check your email" success state within the same card (do not route to a new page). 
*   The success state should say: "A login link has been sent to [email]. Click the link to securely sign in." and have a "Back to login" text button.

---

## 2. Route: `/auth/error` (Error Page)

**Purpose:** If the magic link expires or Auth.js throws an error, they land here.

**Required UI Elements:**
*   A centered Error Card (Use destructive/red styling cues).
*   **Header:** "Authentication Error"
*   **Subtext:** Display a dynamic error message (e.g., "This login link has expired or is invalid.")
